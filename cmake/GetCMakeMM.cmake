...
#  :keyword VERBOSITY: Specify the verbosity.
#  :type VERBOSITY: string ALL TRACE DEBUG INFO WARN ERROR FATAL NONE.
#]]
function(cmmm)
  ...
  cmake_parse_arguments(CMMM "NO_COLOR;SHOW_PROGRESS" "VERBOSITY;VERSION;DESTINATION;INACTIVITY_TIMEOUT;TIMEOUT;TLS_VERIFY;TLS_CAINFO;RETRIES" "" "${ARGN}")

  if(WIN32 OR DEFINED ENV{CLION_IDE} OR DEFINED ENV{DevEnvDir} OR DEFINED ENV{workspaceRoot} OR CMMM_NO_COLOR)
  ...
  list(APPEND CMMM_VERBOSITY_LIST "ALL;TRACE;DEBUG;INFO;WARN;ERROR;FATAL;NONE")
  if(NOT DEFINED CMMM_VERBOSITY)
    set(CMMM_VERBOSITY "ALL")
  else()
    list(FIND CMMM_VERBOSITY_LIST ${CMMM_VERBOSITY} RESULT)
    if("${RESULT}" STREQUAL "-1")
      message(STATUS "${Esc}${CMMM_WARN_COLOR}[ CMMM ] [  warn  ] VERBOSITY parameter ${CMMM_VERBOSITY} not in [ALL, TRACE, DEBUG, INFO, WARN, ERROR, FATAL, NONE].\n   Define VERBOSITY to ALL.${Esc}${CMMM_RESET_COLOR}")
      set(CMMM_VERBOSITY "ALL")
    endif()
  endif()
  list(FIND CMMM_VERBOSITY_LIST ${CMMM_VERBOSITY} CMMM_VERBOSITY_NUM)
  ...
  if(${CMMM_TLS_VERIFY} STREQUAL "ON" OR ${CMMM_TLS_VERIFY} STREQUAL "OFF")
    ...
    if("${CMMM_VERBOSITY_NUM}" LESS_EQUAL "6")
      message(STATUS "${Esc}${CMMM_FATAL_ERROR_COLOR}[ CMMM ] [  fatal ] TLS_VERIFY must have value ON or OFF.${Esc}${CMMM_RESET_COLOR}")
    endif()
  ...
  while(NOT CMAKEMM_INITIALIZED_${CMMM_TAG} OR NOT EXISTS "${CMMM_DESTINATION}/CMakeMM-${CMMM_TAG}.cmake" OR "${CMakeMMSHA256}" STREQUAL "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855")
    ...
    if(${CMMM_RETRIES_DONE} STREQUAL "0")
      ...
      if("${CMMM_VERBOSITY_NUM}" LESS_EQUAL "3")
        message(STATUS "${Esc}${CMMM_INFO_COLOR}[ CMMM ] [  info  ] Retry downloading CMakeMM.cmake (${CMMM_RETRIES_DONE}/${CMMM_RETRIES}).${Esc}${CMMM_RESET_COLOR}")
      endif()
    endif()

    file(
      DOWNLOAD "${CMMM_URL}/${CMMM_TAG}/CMakeMM.cmake2" "${CMMM_DESTINATION}/CMakeMM-${CMMM_TAG}.cmake"
    ...
    if(${CMAKECM_CODE})
      if("${CMMM_VERBOSITY_NUM}" LESS_EQUAL "5")
        message(STATUS "${Esc}${CMMM_ERROR_COLOR}[ CMMM ] [  error ] Error downloading CMakeMM.cmake : ${CMAKECM_MESSAGE} (${CMAKECM_CODE}).${Esc}${CMMM_RESET_COLOR}")
      endif()
    else()
      file(SHA256 "${CMMM_DESTINATION}/CMakeMM-${CMMM_TAG}.cmake" CMakeMMSHA256)
      if(${CMakeMMSHA256} STREQUAL "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855")
        ...
        if("${CMMM_VERBOSITY_NUM}" LESS_EQUAL "5")
          message(STATUS "${Esc}${CMMM_ERROR_COLOR}[ CMMM ] [  error ] Error downloading CMakeMM.cmake : Empty file.${Esc}${CMMM_RESET_COLOR}")
        endif()
      else()
        break()
      endif()
    ...
    if("${CMMM_RETRIES_DONE}" STREQUAL "${CMMM_RETRIES}")
      unlock()
      if("${CMMM_VERBOSITY_NUM}" LESS_EQUAL "6")
        message(STATUS "${Esc}${CMMM_FATAL_ERROR_COLOR}[ CMMM ] [  fatal ] Error downloading CMakeMM.cmake.${Esc}${CMMM_RESET_COLOR}")
      endif()
